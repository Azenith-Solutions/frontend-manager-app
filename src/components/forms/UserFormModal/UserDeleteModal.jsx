...
const UserDeleteModal = ({ open, onClose, user, onUserDeleted }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setSnackbar({
        open: true,
        message: `Usuário ${user?.nome || user?.fullName} excluído com sucesso!`,
        severity: 'success'
      });

      // Simula um processo de finalização
      setTimeout(() => {
    ...
    >      <DialogTitle sx={{
      ...
      borderBottom: '1px solid #e0e0e0'
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>          <Box
          sx={{
            backgroundColor: '#61131A', width: '28px',
            height: '28px',
            borderRadius: '6px',
            display: 'flex',
            alignItems: 'center',
        ...
        </Box>          <Typography variant="h6" sx={{ fontWeight: 600, fontSize: '0.95rem', color: '#333' }}>
            Excluir Usuário
          </Typography>
        </Box>
        <IconButton
      ...
      </DialogTitle>      <DialogContent sx={{ p: 1.75, position: 'relative' }}>
        {/* Overlay de sucesso */}
        {deleteSuccess && (
          <Box
            sx={{
          ...
          >            <DeleteIcon
            ...
            </Typography>            <CircularProgress size={18} sx={{ mt: 0.5, color: '#61131A' }} /></Box>)}            <Typography variant="body1" color="text.primary" sx={{ mb: 1, mt: 0.75, fontWeight: 500, textAlign: 'center', fontSize: '0.9rem' }}>
          Deseja excluir o usuário <strong>{user?.nome || user?.fullName || "selecionado"}</strong>?
        </Typography>        <Box
          sx={{
            mt: 1.5,
            p: 1.25,
        ...
        ><Typography variant="body2" sx={{ color: '#61131A', fontSize: '0.75rem' }}>
            Confirme esta ação apenas se você tem certeza que deseja remover este usuário do sistema.
          </Typography>
        </Box>
      </DialogContent>
      <DialogActions sx={{
      ...
      >
        ...
        >
          {loading ? (
            ...
            />
              <span style={{ visibility: 'hidden' }}>Excluir Usuário</span>
            </>
          ) : deleteSuccess ? (
            'Usuário Excluído'
          ) : (
